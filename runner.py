...
from model import *
from my_noise_data_cifar_10_dataloader import *
import torch
import torchvision
...
def main(writer):
    ...
    lr_list = [0.1] * 40 + [0.01] * 40 + [0.001] * 40 + [1e-4] * 40 + [1e-5] * 40 + [1e-6] * 40 + [1e-7] * 40 + [
        1e-8] * 20
    ...
    for epoch in range(num_epochs):
        print("epoch=", epoch, 'opt1=', args.opt1, 'opt2=', args.opt2, 'opt3=', args.opt3)
        ...
        train(train_loader=train_loader_noisy, peer_loader=peer_train, model=model_PL, optimizer=optimizer_PL,
              epoch=epoch)
        ...
        if best_val_acc <= valid_acc:
            ...
            # save beat model
            torch.save(model_PL, './trained_models/' + str(args.opt1) + '_' + str(args.opt2) + '_' + str(args.opt3))
            print("saved, the accuracy of validation noisy increases.")

        # save more model_PL.state_dict() by epoch
        if epoch % 15 == 0:
            torch.save(model_PL.state_dict(), './trained_models/' + str(args.opt1) + '_' + str(args.opt2) + '_' + str(args.opt3) + '_' + str(epoch/15))
            print(epoch/15)

        writer.writerow([epoch, train_acc, valid_acc, test_acc])
        x = np.linspace(0, epoch, epoch+1)
        vis.line(X=x, Y=train_acc_result, win="1")# 不使用update='append'
        vis.line(X=x, Y=val_acc_noisy_result, win="2")
        vis.line(X=x, Y=test_acc_result, win="3")
...
if __name__ == '__main__':
    ...
    main(writer1)
    evaluate('./trained_models/' + str(args.opt1) + '_' + str(args.opt2) + '_' + str(args.s))
